...
figure
global beta sigma lambda betap sigmap kappa kappap rho nu tau delta;
...
for i = 1:length(nu_values)
    for j = length(lambda_values)
        nu = nu_values(i);
        lambda = lambda_values(j);
        state = [N-Z0-M0,0,Z0, M0, 0].';

        [t,state] = ode45(@(t,y) derivative(t,y),runningTime,state, opts);
...
global beta sigma sigmap tau nu lambda betap rho delta kappap kappa
S = state(1);
I = state(2);
Z = state(3);
M = state(4);
